...
export default function Home() {
  ...
  return (
    <main>
      ...
      {adminOpen && <AdminPanel tab={adminTab} setTab={setAdminTab} close={() => setAdminOpen(false)} products={products} setProducts={setProducts} publications={publications} setPublications={setPublications} />}
...
function AdminPanel({ tab, setTab, close, products, setProducts, publications, setPublications }: {
  ...
  function saveProduct(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const d=new FormData(e.currentTarget);const p:Product={id:editingProduct?.id??Date.now(),name:String(d.get("name")),category:String(d.get("category")),brand:String(d.get("brand")),type:String(d.get("type")),price:Number(d.get("price")),oldPrice:Number(d.get("oldPrice"))||undefined,stock:Number(d.get("stock")),image:String(d.get("image"))||initialProducts[0].image,badge:String(d.get("badge"))||undefined,description:String(d.get("description"))};setProducts(list=>editingProduct?list.map(x=>x.id===p.id?p:x):[p,...list]);setEditor(null)}
  ...
  const openProduct=(p:Product|null)=>{setEditingProduct(p);setEditor("product")}; const openPublication=(p:Publication|null)=>{setEditingPublication(p);setEditor("publication")};
  return <div className="admin-shell">
    ...
    {editor&&<div className="admin-editor-backdrop" onMouseDown={()=>setEditor(null)}><section className="admin-editor" onMouseDown={e=>e.stopPropagation()}><button className="close" onClick={()=>setEditor(null)}><Icon name="x"/></button>
      {editor==="product"&&<form onSubmit={saveProduct}><span className="eyebrow">Catálogo</span><h2>{editingProduct?"Editar produto":"Cadastrar produto"}</h2><div className="editor-fields"><label>Nome<input name="name" required defaultValue={editingProduct?.name}/></label><label>Categoria<select name="category" defaultValue={editingProduct?.category}>{categories.slice(1).map(c=><option key={c}>{c}</option>)}</select></label><label>Marca<input name="brand" defaultValue={editingProduct?.brand}/></label><label>Tipo<input name="type" defaultValue={editingProduct?.type}/></label><label>Preço<input name="price" type="number" step=".01" required defaultValue={editingProduct?.price}/></label><label>Preço anterior<input name="oldPrice" type="number" step=".01" defaultValue={editingProduct?.oldPrice}/></label><label>Estoque<input name="stock" type="number" required defaultValue={editingProduct?.stock}/></label><label>Selo<input name="badge" defaultValue={editingProduct?.badge}/></label><label className="wide">URL da foto<input name="image" defaultValue={editingProduct?.image}/></label><label className="wide">Descrição<textarea name="description" required defaultValue={editingProduct?.description}/></label></div><button className="button primary full">Salvar e atualizar vitrine</button></form>}
